...
function extractPublicCandidates(html,pageUrl){
 const d=new DOMParser().parseFromString(html,'text/html'),out=[],seen=new Set();
...
function nearbyAnchor(node){if(!node)return null;if(node.matches?.('a[href]'))return node;const inner=node.querySelector?.('a[href]');if(inner)return inner;const close=node.closest?.('a[href]');if(close)return close;let s=node.previousElementSibling;for(let i=0;s&&i<4;i++,s=s.previousElementSibling){if(s.matches?.('a[href]'))return s;const a=s.querySelector?.('a[href]');if(a)return a}return node.parentElement?.querySelector?.('a[href]')||null}

function albumItems(doc,pageUrl){
 const base=new URL(pageUrl),found=[],seen=new Set();
 function add(node,title=''){
  ...
  const a=nearbyAnchor(node),href=a?norm(a.getAttribute('href'),pageUrl):'';
...
async function probeDownload(url){
 const result={original:url,finalUrl:url,status:'valid',contentType:'',redirected:false};
 try{
  let r;
  try{r=await fetch(url,{method:'HEAD',mode:'cors',credentials:'omit',redirect:'follow',cache:'no-store'})}
  catch{r=await fetch(url,{method:'GET',mode:'cors',credentials:'omit',redirect:'follow',cache:'no-store',headers:{Range:'bytes=0-0'}})}
  if(!r.ok)throw new Error(`HTTP ${r.status}`);
  result.finalUrl=r.url||url;
  result.redirected=result.finalUrl!==url;
  result.contentType=(r.headers.get('content-type')||'').toLowerCase();
  result.status=result.contentType.startsWith('video/')||videoExt.test(result.finalUrl)?'media':(result.redirected?'redirect':'valid');
 }catch(error){result.status='opaque';result.error=error?.message||'Resposta não legível pelo navegador'}
 return result;
}

async function resolveEntry(e){
 if(e.download){const probe=await probeDownload(e.download);return{...e,url:probe.finalUrl||e.download,kind:'download',playable:true,probe}}
 try{const r=await fetchPage(e.sourcePage);const type=(r.headers.get('content-type')||'').toLowerCase();if(type.startsWith('video/'))return{...e,url:r.url,kind:'direct',playable:true};const p=extractPublicCandidates(await r.text(),r.url||e.sourcePage);const c=p.candidates.find(x=>x.kind==='download')||p.candidates.find(x=>x.kind==='direct')||p.candidates.find(x=>x.kind==='embed');if(c?.kind==='download'){const probe=await probeDownload(c.url);return{...e,url:probe.finalUrl||c.url,kind:'download',playable:true,probe}}return{...e,url:c?.url||'',kind:c?.kind||'',playable:Boolean(c?.url)}}catch(err){return{...e,url:'',kind:'',playable:false,error:err?.message||'Falha'}}
}

async function discover(html,pageUrl){const p=extractPublicCandidates(html,pageUrl),cards=albumItems(p.doc,pageUrl);if(!cards.length){const c=p.candidates.find(x=>x.kind==='download')||p.candidates[0];if(c?.kind==='download'){const probe=await probeDownload(c.url);return{title:p.title,items:[{title:p.title,sourcePage:pageUrl,url:probe.finalUrl||c.url,kind:'download',playable:true,probe}]}}return{title:p.title,items:[{title:p.title,sourcePage:pageUrl,url:c?.url||'',kind:c?.kind||'',playable:Boolean(c?.url)}]}}setStatus(`Preparando ${cards.length} vídeo(s)…`);return{title:p.title,items:await Promise.all(cards.map(resolveEntry))}}

function renderPlaylist(){playlist.innerHTML='';items.forEach((it,i)=>{const b=document.createElement('button');b.type='button';b.className=`media-item${i===currentIndex?' active':''}`;const th=document.createElement('span');th.className='thumb';th.textContent=it.playable?'▶':'—';const wrap=document.createElement('span'),n=document.createElement('span'),ty=document.createElement('span');n.className='media-name';n.textContent=it.title||`Vídeo ${i+1}`;ty.className='media-type';if(it.kind==='download'){ty.textContent=it.probe?.status==='media'?'Vídeo • mídia final encontrada':it.probe?.status==='redirect'?'Vídeo • redirecionamento encontrado':it.probe?.status==='opaque'?'Vídeo • link válido, resposta protegida pelo navegador':'Vídeo • link de arquivo'}else ty.textContent=it.playable?(it.kind==='embed'?'Vídeo • player público':'Vídeo • fonte direta'):'Vídeo • identificado';wrap.append(n,ty);b.append(th,wrap);b.onclick=()=>selectItem(i,true);playlist.appendChild(b)})}
...
function showOpenFallback(it){imageViewer.hidden=false;imageViewer.innerHTML='';const p=document.createElement('p');p.textContent=it.probe?.status==='opaque'?'O link de download existe, mas o navegador não permite ler o redirecionamento por CORS.':'O endereço foi aceito, mas o navegador não iniciou a reprodução direta.';const a=document.createElement('a');a.href=it.probe?.original||it.url;a.target='_blank';a.rel='noopener noreferrer';a.textContent='Abrir arquivo diretamente';a.style.display='inline-block';a.style.marginTop='10px';imageViewer.append(p,a)}
function selectItem(i,autoplay=false){if(!items.length)return;currentIndex=Math.max(0,Math.min(i,items.length-1));const it=items[currentIndex];currentTitle.textContent=it.title||`Vídeo ${currentIndex+1}`;position.textContent=`${currentIndex+1} de ${items.length}`;prevButton.disabled=currentIndex===0;nextButton.disabled=currentIndex===items.length-1;resetPlayer();if(!it.playable||!it.url){player.hidden=true;imageViewer.hidden=false;imageViewer.textContent='Vídeo identificado, mas não encontrei uma fonte reproduzível.';setStatus('Fonte não encontrada para este item.','error');renderPlaylist();return}if(it.kind==='embed'){showEmbed(it.url);renderPlaylist();return}player.hidden=false;if(it.kind==='download'){if(it.probe?.status==='media')setStatus('Redirecionamento concluído; carregando mídia final…');else if(it.probe?.status==='redirect')setStatus('Redirecionamento encontrado; tentando reprodução…');else if(it.probe?.status==='opaque')setStatus('Link de download válido; o navegador não expôs o destino final por CORS.');else setStatus('Abrindo link de arquivo…')}else setStatus('Carregando vídeo…');player.src=it.url;player.load();if(autoplay)player.play().catch(()=>{});renderPlaylist()}
