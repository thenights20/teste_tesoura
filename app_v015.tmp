...
function extractPublicCandidates(html,pageUrl){
 ...
 d.querySelectorAll('a[href]').forEach(a=>{const u=norm(a.getAttribute('href'),pageUrl);if(u&&videoExt.test(u))add(u,a.textContent||docTitle(d))});
...
async function resolveEntry(e){
 ...
 try{const r=await fetchPage(e.sourcePage);const type=(r.headers.get('content-type')||'').toLowerCase();if(type.startsWith('video/'))return {...e,url:r.url,kind:'direct',playable:true};const p=extractPublicCandidates(await r.text(),r.url||e.sourcePage);const c=p.candidates.find(x=>x.kind==='direct')||p.candidates.find(x=>x.kind==='embed');return {...e,url:c?.url||'',kind:c?.kind||'',playable:Boolean(c?.url)}}catch(err){return {...e,url:'',kind:'',playable:false,error:err?.message||'Falha'}}
}

async function discover(html,pageUrl){
 const p=extractPublicCandidates(html,pageUrl); const cards=albumItems(p.doc,pageUrl);
 if(!cards.length){const c=p.candidates[0];return {title:p.title,items:[{title:p.title,sourcePage:pageUrl,url:c?.url||'',kind:c?.kind||'',playable:Boolean(c?.url)}]}}
